...
class StarCraft2Env(MultiAgentEnv):
    ...
    def _launch(self):
        ...
        create = sc_pb.RequestCreateGame(local_map=sc_pb.LocalMap(map_path=_map.path,
        ...
        self.terrain_height = np.flip(
            np.transpose(np.array(list(map_info.terrain_height.data)).reshape(self.map_x, self.map_y)), 1) / 255
    ...
    def get_obs_agent(self, agent_id):
        ...
        if self.obs_agent_id:
            ...
            agent_obs = np.concatenate((ally_feats.flatten(), enemy_feats.flatten(), move_feats.flatten(),
                                        own_feats.flatten(), agent_id_feats.flatten()))
    ...
    def get_state_agent(self, agent_id):
        ...
        state = np.concatenate((ally_feats.flatten(), enemy_feats.flatten(), move_feats.flatten(), own_feats.flatten()))
    ...
    def get_obs_size(self):
        ...
        return [
            all_feats * self.stacked_frames if self.use_stacked_frames else all_feats, [n_allies, n_ally_feats],
    ...
    def get_state_size(self):
        ...
        return [
            size * self.stacked_frames if self.use_stacked_frames else size, [self.n_agents, nf_al],
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        if unit.health > 0:
            ...
            if self.map_type == "MMM" and unit.unit_type == self.medivac_id:
                ...
                target_items = [(t_id, t_unit) for (t_id, t_unit) in self.agents.items()
