...
from onpolicy.envs.hns.wrappers.util import DiscretizeActionWrapper, DiscardMujocoExceptionEpisodes
from onpolicy.envs.hns.wrappers.line_of_sight import AgentAgentObsMask2D
from onpolicy.envs.hns.modules.agents import Agents
from onpolicy.envs.hns.modules.walls import RandomWalls
from onpolicy.envs.hns.modules.objects import Boxes, Ramps
...
class Base(Env):
    ...
    def _get_sim(self, seed):
        ...
        '''
        world_params = WorldParams(size=(self.floor_size, self.floor_size, 2.5), num_substeps=self.n_substeps)
        ...
        while not successful_placement:
            ...
            successful_placement = np.all(
                [module.build_world_step(self, floor, self.floor_size) for module in self.modules])
            failures += 1
...
def make_env(n_substeps=5, horizon=250, deterministic_mode=False, n_agents=2, n_boxes=2, n_ramps=1):
    '''
    ...
    '''
    env = Base(n_agents=n_agents, n_substeps=n_substeps, horizon=horizon, deterministic_mode=deterministic_mode)
