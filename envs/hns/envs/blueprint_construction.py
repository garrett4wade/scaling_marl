...
class ConstructionDistancesWrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        boxcorner_sitecorner_dist = (np.linalg.norm(boxcorner_pos[..., None] - sitecorner_pos.T[None, ...], axis=1))

        obs.update({
            'box_box_dist': box_box_dist,
...
class ConstructionDenseRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        box_site_dist = (obs['boxcorner_sitecorner_dist'] if self.use_corners else obs['box_site_dist'])
        scaling_factors = np.exp(self.alpha * box_site_dist)
        site_box_smoothmin_dists = (np.sum(box_site_dist * scaling_factors, axis=0) / np.sum(scaling_factors, axis=0))
...
class ConstructionCompletedRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        construction_completed = ((all_sites_activated and not self.use_corners)
                                  or (all_sites_activated and all_corners_aligned))
...
def make_env(n_substeps=15,
    ...
    if scenario == 'randomwalls':
        ...
        env.add_module(
            WallScenarios(grid_size=grid_size, door_size=door_size, scenario='empty', friction=other_friction))
