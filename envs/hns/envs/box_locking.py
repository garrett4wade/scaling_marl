...
from envs.hns.envs.hide_and_seek import quadrant_placement
...
class LockObjectsTask(gym.Wrapper):
    ...
    def __init__(self,
        ...
        assert task in ['all', 'order', 'all-return', 'order-return'], (f'task {task} is currently not supported')
    ...
    def _get_next_obj(self, obs):
        ...
        if len(self.unlocked_objs) == 0:
            ...
            obj_dist = [(np.linalg.norm(obs[self.pos_key][i, :2] - agent_pos), i) for i in self.unlocked_objs]
    ...
    def step(self, action):
        ...
        if n_unlocked == 0 and ((not self.need_return) or self.spawn_pos_dist <= self.return_threshold):
...
def tri_placement(tri_room_idx):
    ...
    def placement(grid, obj_size, metadata, random_state):
        ...
        pos = np.array([
            random_state.randint(x_rag[0], x_rag[1] - obj_size[0]),
...
def make_env(
    ...
    if grab_box and np.max(n_boxes) > 0:
        body_names = ([f'moveable_box{i}' for i in range(n_boxes)] + [f"ramp{i}:ramp" for i in range(n_ramps)])
