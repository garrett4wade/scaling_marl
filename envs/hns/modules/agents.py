...
from envs.hns.util.transforms import (add_weld_equality_constraint_transform, set_joint_damping_transform)
...
class Agents(EnvModule):
    ...
    def build_world_step(self, env, floor, floor_size):
        ...
        for i in range(self.n_agents):
            ...
            if self.placement_fn is not None:
                ...
                # if 'set_agent_state' in env.metadata:
                #     pos, pos_grid = rejection_placement_agent(env, i, _placement_fn, floor_size, obj_size)
                # else:
                pos, pos_grid = rejection_placement(env, i, _placement_fn, floor_size, obj_size)
                if pos is not None:
    ...
    #             obj_size = get_size_from_xml(obj)
    #             if 'set_agent_state' in env.metadata:
    #                 if 'uniform_num_hider' in env.metadata: # hide and seek
    #                     if i >= env.metadata['uniform_num_hider']: # seeker
    #                         pos, pos_grid = rejection_placement_agent(env, i, _placement_fn, floor_size, obj_size)
    #                     else:
    ...
    def observation_step(self, env, sim):
        ...
        for i in range(self.n_agents):
            env.metadata[f'agent{i}_pos'] = np.rint(agent_qpos[i, :2] * env.grid_size / env.floor_size).astype(np.int16)
...
#     '''
#     @store_args
#     def __init__(self, n_agents, placement_fn=None, color=None, friction=None, damp_z=False, polar_obs=True):
#         pass
...
#                 obj.add_transform(set_geom_attr_transform('friction', self.friction))
#             if self.color is not None:
#                 _color = (self.color[i] if isinstance(self.color[0], (list, tuple, np.ndarray)) else self.color)
#                 obj.add_transform(set_geom_attr_transform('rgba', _color))
#             if not self.damp_z:
#                 obj.add_transform(set_joint_damping_transform(1, 'tz'))
...
#     def modify_sim_step(self, env, sim):
#         # Cache qpos, qvel idxs
#         self.agent_qpos_idxs = np.array([qpos_idxs_from_joint_prefix(sim, f'agent{i}') for i in range(self.n_agents)])
#         self.agent_qvel_idxs = np.array([qvel_idxs_from_joint_prefix(sim, f'agent{i}') for i in range(self.n_agents)])
#         env.metadata['agent_geom_idxs'] = [sim.model.geom_name2id(f'agent{i}:agent') for i in range(self.n_agents)]
...
#         for i in range(env.n_agents):
#             floor.add_transform(
#                 add_weld_equality_constraint_transform(f'agent{i}:gripper', f'agent{i}:particle', 'floor0'))
#         return True
