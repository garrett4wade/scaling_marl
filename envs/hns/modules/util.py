...
def uniform_placement(grid, obj_size, metadata, random_state):
    grid_size = len(grid)
    pos = np.array(
        [random_state.randint(1, grid_size - obj_size[0] - 1),
...
def close_to_other_object_placement(object_type, object_index, radius_key):
    def close_placement_fn(grid, obj_size, metadata, random_state):
        ...
        pos = np.array([
            random_state.randint(distr_limits_min[0], distr_limits_max[0]),
...
def uniform_placement_middle(area_side_length_fraction):
    ...
    def uniform_placement_middle_fn(grid, obj_size, metadata, random_state):
        grid_size = len(grid)
        distr_limits_min = ((grid_size - obj_size) * (1 - area_side_length_fraction) / 2 +
                            area_side_length_fraction).astype(int)
        distr_limits_max = ((grid_size - obj_size) * (1 + area_side_length_fraction) / 2 -
                            area_side_length_fraction).astype(int)
...
def center_placement(grid, obj_size_in_cells, metadata, random_state):
    ...
    pos = np.array([half_grid_size - int(obj_size_in_cells[0] / 2), half_grid_size - int(obj_size_in_cells[1] / 2)])
    return pos
