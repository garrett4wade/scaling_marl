...
class Wall:
    ...
    def split_for_doors(self, num_doors=1, door_size=1, all_connect=False, random_state=np.random.RandomState()):
        '''
...
def choose_new_split(walls, min_dist_between, num_tries=10, random_state=np.random.RandomState()):
    ...
    for i in range(num_tries):
        ...
        proposed_walls = [
            connect_walls(wall1, wall2, min_dist_between, random_state=random_state) for wall2 in walls
            if wall2 != wall1
        ]
        proposed_walls = [
            wall for wall in proposed_walls
            if wall is not None and not np.any([wall.intersects(_wall) for _wall in walls])
        ]
...
def outside_walls(grid_size, rgba=(0, 1, 0, 0.1), use_low_wall_height=False):
    ...
    return [
        Wall([0, 0], [0, grid_size - 1], height=height, rgba=rgba),
...
class RandomWalls(EnvModule):
    ...
    def build_world_step(self, env, floor, floor_size):
        # Create rooms
        walls = outside_walls(self.grid_size, rgba=self.outside_wall_rgba, use_low_wall_height=self.low_outside_walls)
        failures = 0
        ...
        while len(walls) < self.num_actual_rooms + 3:
            ...
            if new_wall is None:
                walls = outside_walls(self.grid_size,
                                      rgba=self.outside_wall_rgba,
