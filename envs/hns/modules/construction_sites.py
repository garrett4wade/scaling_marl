...
from onpolicy.envs.hns.modules import EnvModule, rejection_placement
...
class ConstructionSites(EnvModule):
    ...
    def _mark_site_square(self, floor, floor_size, site_name, site_relative_xyz, site_dims):
        x, y, z = site_relative_xyz
        floor.mark(site_name, relative_xyz=(x, y, z), rgba=[1., 1., 1., 1.], size=0.1)
    ...
    def build_world_step(self, env, floor, floor_size):
        ...
        self.curr_n_elongated_sites = env._random_state.randint(self.n_elongated_sites[0],
                                                                self.n_elongated_sites[1] + 1)
        ...
        if self.curr_n_elongated_sites > 0:
            ...
            self.site_size_array[n_xaligned:self.curr_n_elongated_sites, :] = (self.site_size * np.array([0.3, 3.3]))
        ...
        for i in range(self.curr_n_sites):
            if self.placement_fn is not None:
                ...
                pos, _ = rejection_placement(env, _placement_fn, floor_size, self.site_size_array[i])
                if pos is not None:
                    self._mark_site_square(floor, floor_size, f'{self.site_name}{i}',
                                           (pos[0], pos[1], self.site_height), self.site_size_array[i])
                else:
                ...
                self._mark_site_square(floor, floor_size, f'{self.site_name}{i}', (pos[0], pos[1], self.site_height),
    ...
    def modify_sim_step(self, env, sim):
        self.construction_site_idxs = np.array(
            [sim.model.site_name2id(f'{self.site_name}{i}') for i in range(self.curr_n_sites)])
        self.construction_site_corner_idxs = np.array([
            sim.model.site_name2id(f'{self.site_name}{i}_corner{j}') for i in range(self.curr_n_sites) for j in range(4)
        ])
    ...
    def observation_step(self, env, sim):
        ...
        site_obs = np.concatenate((site_pos, site_corner_pos.reshape((self.curr_n_sites, 12))), axis=-1)

        mask_site_obs = np.ones((env.n_agents, self.curr_n_sites))

        obs = {
            'construction_site_pos': site_pos,
