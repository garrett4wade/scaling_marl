...
class Cylinders(EnvModule):
    ...
    '''
    @store_args
    def __init__(self, n_objects, diameter, height, make_static=False, placement_fn=None, rgba=[1., 1., 1., 1.]):
    ...
    def modify_sim_step(self, env, sim):
        if self.make_static:
            self.s_cylinder_geom_idxs = np.array(
                [sim.model.geom_name2id(f'static_cylinder{i}') for i in range(self.n_objects)])
        else:
            self.m_cylinder_geom_idxs = np.array(
                [sim.model.geom_name2id(f'moveable_cylinder{i}') for i in range(self.n_objects)])
            qpos_idxs = [qpos_idxs_from_joint_prefix(sim, f'moveable_cylinder{i}') for i in range(self.n_objects)]
            qvel_idxs = [qvel_idxs_from_joint_prefix(sim, f'moveable_cylinder{i}') for i in range(self.n_objects)]
    ...
    def observation_step(self, env, sim):
        ...
        if self.make_static:
            ...
            obs = {'static_cylinder_geom_idxs': s_cylinder_geom_idxs, 'static_cylinder_xpos': s_cylinder_xpos}
        else:
...
class LidarSites(EnvModule):
    ...
    def modify_sim_step(self, env, sim):
        ...
        self.lidar_ids = np.array(
            [[sim.model.site_name2id(f"agent{i}:lidar{j}") for j in range(self.n_lidar_per_agent)]
             for i in range(self.n_agents)])
