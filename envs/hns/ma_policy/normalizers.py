...
class EMAMeanStd(object):
    ...
    def __init__(self, beta, scope="ema", reuse=None, epsilon=1e-6, per_element_update=False, shape=(), version=1):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            # Expected value of x
            self._biased_mean = tf.get_variable(dtype=tf.float32,
                                                shape=shape,
                                                initializer=tf.constant_initializer(0.0),
                                                name="mean",
                                                trainable=False)
            # Expected value of x^2
            self._biased_sq = tf.get_variable(dtype=tf.float32,
                                              shape=shape,
                                              initializer=tf.constant_initializer(0.0),
                                              name="sq",
                                              trainable=False)
            ...
            self._debiasing_term = tf.get_variable(dtype=tf.float32,
                                                   shape=shape,
                                                   initializer=tf.constant_initializer(0.0),
                                                   name="debiasing_term",
                                                   trainable=False)
            ...
            self.std = _std_from_mean_and_square(mean=self.mean,
                                                 square=self._biased_sq / tf.maximum(self._debiasing_term, epsilon))
    ...
    def update_op(self, x, axes=(0, )):
        ...
        return tf.group(
            ...
            tf.assign(
                self._debiasing_term,
                tf.to_float(
                    _interpolate(old=tf.cast(self._debiasing_term, tf.float64),
                                 new=one,
            ...
            tf.assign(
                self._biased_mean,
                _interpolate(old=self._biased_mean,
                             new=tf.reduce_mean(tf.to_float(x), axis=axes),
                             old_weight=old_weight_fp32,
                             scaled_weight=scaled_weight_fp32)),
            # do an interpolation on the expected value of X^2
            tf.assign(
                self._biased_sq,
                _interpolate(old=self._biased_sq,
                             new=tf.reduce_mean(tf.square(tf.to_float(x)), axis=axes),
                             old_weight=old_weight_fp32,
                             scaled_weight=scaled_weight_fp32)),
        )
