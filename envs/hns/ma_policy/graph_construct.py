...
from ma_policy.layers import (entity_avg_pooling_masked, entity_max_pooling_masked, entity_concat, concat_entity_masks,
                              residual_sa_block, circ_conv1d)
                ...
                with tf.variable_scope(extra_layer_scope, reuse=reuse):
                    if layer_type == 'dense':
                        assert len(nodes_in) == len(
                            nodes_out), f"Dense layer must have same number of nodes in as nodes out. \
                        ...
                        with tf.variable_scope(layer_name):
                            ...
                            assert (len(shape_list(inp[nodes_in[0]])) == len(shape_list(inp[nodes_in[1]]))
                                    or len(shape_list(inp[nodes_in[0]])) == len(shape_list(inp[nodes_in[1]])) -
                                    1), (f"shapes were {nodes_in[0]}:{shape_list(inp[nodes_in[0]])}"
                                         f", {nodes_in[1]}:{shape_list(inp[nodes_in[1]])}")
                        ...
                        with tf.variable_scope(layer_name):
                            ...
                            inp[nodes_out[0]] = residual_sa_block(sa_inp,
                                                                  mask,
                                                                  **layer,
                        ...
                        assert len(nodes_in) == len(
                            nodes_out) == 1, f"Circular convolution layer must have one nodes and one nodes out. \
                            Nodes in: {nodes_in}, Nodes out {nodes_out}"
