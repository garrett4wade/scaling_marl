...
class MAPolicy:
    ...
    def __init__(self,
        ...
        self.input_schemas = {
            k: VariableSchema(shape=[BATCH, TIMESTEPS] + pdtype.sample_shape(), dtype=pdtype.sample_dtype())
            for k, pdtype in self.pdtypes.items()
        }

        # Creat input schemas for each observation
        for k, v in self.ob_space.spaces.items():
            self.input_schemas[k] = VariableSchema(shape=[BATCH, TIMESTEPS] + list(v.shape), dtype=tf.float32)

        # Setup schemas and zero state for layers with state
        v_state_schemas, v_zero_states = construct_schemas_zero_state(self.v_network_spec, self.ob_space, 'vpred_net')
        pi_state_schemas, pi_zero_states = construct_schemas_zero_state(self.network_spec, self.ob_space, 'policy_net')
        ...
        if build_act:
            with tf.variable_scope(self.scope, reuse=self.reuse):
                self.phs = {name: schema.placeholder(name=name) for name, schema in self.get_input_schemas().items()}
            self.build(self.phs)
    ...
    def _init(self, inputs, gaussian_fixed_var=True, **kwargs):
        ...
        # Value network
        (vpred, vpred_state_out, vpred_reset_ops) = construct_tf_graph(processed_inp,
                                                                       self.v_network_spec,
                                                                       scope='vpred_net',
                                                                       act=self.build_act)
        ...
        # Policy network
        (pi, pi_state_out, pi_reset_ops) = construct_tf_graph(processed_inp,
                                                              self.network_spec,
                                                              scope='policy_net',
                                                              act=self.build_act)
                    ...
                    if self.gaussian_fixed_var and isinstance(self.ac_space.spaces[k], gym.spaces.Box):
                        ...
                        if isinstance(self.ac_space.spaces[k], (gym.spaces.Discrete)):
                            ...
                            assert np.prod(pi[k].get_shape()[-2:]) == self.pdtypes[k].param_shape()[0], (
                                f"policy had shape {pi[k].get_shape()} for action {k}"
                                f", but required {self.pdtypes[k].param_shape()}")
    ...
    def _init_vpred_head(self, vpred, processed_inp, vpred_scope, feedback_name):
        with tf.variable_scope(vpred_scope):
            _vpred = tf.layers.dense(vpred['main'],
                                     1,
                                     activation=None,
    ...
    def act(self, observation, extra_feed_dict={}):
        ...
        info = {
            'vpred': preprocess_act_output(outputs['vpred']),
    ...
    def get_trainable_variables(self):
        ...
        if self.trainable_vars is not None:
            variables = [v for v in variables if any([tr_v in v.name for tr_v in self.trainable_vars])]
        elif self.not_trainable_vars is not None:
            variables = [v for v in variables if not any([tr_v in v.name for tr_v in self.not_trainable_vars])]
