...
class SplitMultiAgentActions(gym.ActionWrapper):
    ...
    '''
    def __init__(self, env, flatten=False):
        super().__init__(env)
...
class SelectKeysWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env, keys_self, keys_external, keys_mask=[], flatten=True):
        ...
        if flatten:
            size = sum(
                [np.prod(self.env.observation_space.spaces[k].shape[1:]) for k in self.keys_self + self.keys_external])
            self.observation_space = Dict({'observation_self': Box(-np.inf, np.inf, (size, ), np.float32)})
        else:
            ...
            obs_extern = {
                ...
                for k, v in self.observation_space.spaces.items() if k in self.keys_external + self.keys_mask
            }
    ...
    def observation(self, observation):
        if self.flatten:
            extern_obs = [observation[k].reshape((observation[k].shape[0], -1)) for k in self.keys_external]
