...
class RestrictAgentsRect(gym.RewardWrapper):
    ...
    def __init__(self, env, restrict_rect, reward_scale=10., penalize_objects_out=False):
        ...
        self.rect_middle = 0.5 * np.array([restrict_rect[0] + restrict_rect[2], restrict_rect[1] + restrict_rect[3]])

        self.rect_size = np.array([restrict_rect[2] - restrict_rect[0], restrict_rect[3] - restrict_rect[1]])
    ...
    def reset(self):
        ...
        if self.penalize_objects_out:
            obj_body_idxs = (
                [sim.model.body_name2id(f'moveable_box{i}') for i in np.where(self.metadata['curr_n_boxes'])[0]] +
