...
class TeamMembership(gym.ObservationWrapper):
    ...
    def __init__(self, env, team_index=None, n_teams=2):
        ...
        assert len(team_index) == self.n_agents, ("team_index parameter length must be equal to number of agents")
        if isinstance(team_index, np.ndarray):
            assert team_index.ndim == 1, ("team_index parameter must be numpy array of dimension 1")
    ...
    def observation(self, obs):
        obs['team_size'] = np.sum(self.team_idx[:, None] == self.team_idx[None, :], axis=1, keepdims=True)
        return obs
