...
class Lidar(gym.ObservationWrapper):
    ...
    def __init__(self, env, n_lidar_per_agent=30, lidar_range=6.0, compress_lidar_scale=None, visualize_lidar=False):
        ...
        self.observation_space = update_obs_space(env, {'lidar': (self.n_agents, self.n_lidar_per_agent, 1)})
    ...
    def reset(self):
        ...
        # Cache ids
        self.agent_body_ids = np.array([sim.model.body_name2id(f"agent{i}:particle") for i in range(self.n_agents)])
    ...
    def observation(self, obs):
        ...
        for i in range(self.n_agents):
            for j in range(self.n_lidar_per_agent):
                lidar[i, j] = raycast(sim, geom1_id=self.agent_geom_ids[i], pt2=lidar_endpoints[i, j],
                                      geom_group=None)[0]

        lidar[lidar < 0.0] = self.lidar_range

        if self.compress_lidar_scale is not None:
            obs['lidar'] = (self.compress_lidar_scale * np.tanh(lidar[..., None] / self.compress_lidar_scale))
        else:
