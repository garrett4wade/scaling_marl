...
class GrabObjWrapper(gym.Wrapper):
    ...
    def reset(self):
        ...
        self.agent_eq_ids = np.array(
            [i for i, obj1 in enumerate(sim.model.eq_obj1id) if sim.model.body_names[obj1] == f"agent{i}:particle"])
    ...
    def grab_obj(self, action):
        ...
        new_grabs = np.logical_and(valid_grabs, np.any(self.obj_grabbed != self.last_obj_grabbed, axis=-1))
...
class LockObjWrapper(gym.Wrapper):
    ...
    def __init__(self,
        ...
        self.agent_idx_allowed_to_lock = np.arange(
            self.n_agents) if agent_idx_allowed_to_lock is None else agent_idx_allowed_to_lock
    ...
    def observation(self, obs):
        ...
        obs[f'{self.ac_obs_prefix}you_lock'] = np.expand_dims(you_lock * obs[f'{self.ac_obs_prefix}obj_lock'].T,
                                                              axis=-1)
        ...
        for team in np.unique(self.metadata['team_index']):
            ...
            obs[f'{self.ac_obs_prefix}team_lock'][team_mask] = np.any(obs[f'{self.ac_obs_prefix}you_lock'][team_mask],
                                                                      0)
        return obs
    ...
    def lock_obj(self, action_lock):
        ...
        if self.lock_type == "any_lock":  # If any agent wants to lock, the obj becomes locked
            ...
            allowed_to_unlock = np.logical_and(allowed_to_unlock,
                                               self.obj_locked[None, :])  # Can't unlock an obj that isn't locked
            ...
            for obj in np.argwhere(objs_to_relock)[:, 0]:
                self.which_locked[obj] = np.random.choice(self.agent_idx_allowed_to_lock[np.argwhere(
                    allowed_and_desired[:, obj]).flatten()])
            ...
            allowed_to_unlock = self.metadata['team_index'][:, None] == (self.metadata['team_index'][None,
                                                                                                     self.which_locked])
            allowed_and_not_desired = np.logical_and(allowed_to_unlock[self.agent_idx_allowed_to_lock],
                                                     allowed_and_not_desired)
            objs_to_unlock = np.any(allowed_and_not_desired, axis=0)
...
class LockAllWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env, remove_object_specific_lock=False):
        ...
        self.action_space.spaces['action_glueall'] = (Tuple([Discrete(2) for _ in range(self.n_agents)]))
