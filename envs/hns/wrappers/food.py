...
class FoodHealthWrapper(gym.Wrapper):
    ...
    def __init__(self,
        ...
        self.observation_space = update_obs_space(self.env, {
    ...
    def reset(self):
        ...
        self.food_site_ids = np.array([sim.model.site_name2id(f'food{i}') for i in range(self.curr_n_food)])
        # Reset food healths
    ...
    def observation(self, obs):
        ...
        if self.reward_scale_obs:
            obs['food_obs'] = np.concatenate(
                [obs['food_obs'], np.ones((self.curr_n_food, 1)) * self.curr_reward_scale], 1)
    ...
    def step(self, action):
        ...
        if self.curr_n_food > 0:
            ...
            self.unwrapped.sim.model.site_size[self.food_site_ids[self.respawn_counters ==
                                                                  self.respawn_time]] = self.max_food_size
