    ...
    def run(self):
        ...
        for i, key in enumerate(self.order_obs):
            if key in self.env.observation_space.spaces.keys():
                if self.mask_order_obs[i] is None:
            ...
            with torch.no_grad():
                for agent_id in range(self.num_agents):
                    ...
                    (value, action, action_log_prob, recurrent_hidden_states,
                     recurrent_hidden_states_critic) = self.policies[0].act(
                         agent_id, torch.tensor(self.share_obs[:, agent_id, :]), torch.tensor(self.obs[:, agent_id, :]),
