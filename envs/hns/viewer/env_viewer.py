...
class EnvViewer(MjViewer):
        ...
        if key == glfw.KEY_A:
            if isinstance(current_action_space, Box):
                self.action[self.action_types[self.action_type_mod_index]][self.agent_mod_index][
                    self.action_mod_index] -= 0.05
            elif isinstance(current_action_space, Discrete):
                self.action[self.action_types[self.action_type_mod_index]][self.agent_mod_index] = (
                    (self.action[self.action_types[self.action_type_mod_index]][self.agent_mod_index] - 1) %
                    current_action_space.n)
            elif isinstance(current_action_space, MultiDiscrete):
                self.action[self.action_types[self.action_type_mod_index]][self.agent_mod_index][
                    self.action_mod_index] = ((self.action[self.action_types[self.action_type_mod_index]][
                        self.agent_mod_index][self.action_mod_index] - 1) %
    ...
    def run(self, once=False):
        while True:
            ...
            self.add_overlay(const.GRID_TOPRIGHT, "on agent index %d out %d" % (self.agent_mod_index, self.n_agents),
                             "Y / U")
            self.add_overlay(const.GRID_TOPRIGHT, f"on action type {self.action_types[self.action_type_mod_index]}",
                             "G / B")
            self.add_overlay(
                const.GRID_TOPRIGHT,
                "on action index %d out %d" % (self.action_mod_index, self.num_action[self.action_type_mod_index]),
                "J / K")
