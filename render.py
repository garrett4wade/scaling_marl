...
def main(args):
    ...
    setup = experiment[0].initial_setup()
    ...
    gif_dir = os.path.join("../results/", args.experiment_name, args.trial_name, ckpt_versions[0])
    ...
    for ep in range(args.episodes):
        ...
        while not all(dones):
            ...
            env.render()
            gif.append(display.waitgrab())
            time.sleep(0.02)

            ep_step += 1
            dones = [r.done.all() for r in rs]

        imageio.mimsave(os.path.join(gif_dir ,f"episode_{ep}.gif"), gif, duration=0.1)
